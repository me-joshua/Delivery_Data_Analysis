...
class EDDPredictor:
    ...
    def preprocess_data(self, df):
        ...
        # Create delivery categories for classification
        data['Delivery_Category'] = pd.cut(data['Delivery_Days'], 
                                         bins=[0, 3, 7, 14, float('inf')], 
                                         labels=['Very Fast', 'Fast', 'Normal', 'Slow'])
        
        # Create binary classification for on-time delivery (<=7 days)
    ...
    def train_classification_models(self, data, feature_columns, test_size=0.2, random_state=42):
        ...
        X = data[feature_columns]
        y = data['On_Time']  # Binary classification: on-time vs delayed
...
def main():
    ...
    # Generate delivery dates with realistic patterns
